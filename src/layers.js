...
const setState = (state) => {
  (state.numShapes = state.sides),
    ...
    (state.layerColor = getRandomColorFromPalette());
  return state;
};
...
const simpleLines = (state) => {
  state.numSteps = selectRandomlyTwo()
    ? state.stepsOut
  ...
  state.weight = selectRandomlyTwo() ? state.thinStroke : state.thickStroke;
...
const outlineShape = (state) => {
  state.hexagonTrue = selectRandomlyTwo();
  state.weight = selectRandomlyTwo() ? state.thinStroke : state.thickStroke;
...
const centeredShape = (state) => {
  ...
  state.shapeSize = floor(
    random(state.stepsOut / 2, state.stepsOut - 2) * state.singleStep
  );
...
const spreadedShapes = (state) => {
  state.steps = floor(random(1, state.stepsOut));
  state.center = state.steps * state.singleStep;
  state.randomShape = random(1);
  state.direction = selectRandomlyTwo();
  state.fillColor = selectRandomlyTwo() ? state.layerColor : color(0, 1);
  state.weight = selectRandomlyTwo() ? state.thinStroke : state.thickStroke;
    ...
    render: () => {
      ...
      for (let i = 0; i < state.numShapes; i++) {
        if (state.randomShape < 0.33) {
          ...
          triangles(state.center, state.radius, state.direction);
        }
        rotate(state.angle);
      }
      pop();
...
const steppedHexagons = (state) => {
  state.numSteps = selectRandomlyTwo() ? state.stepsOut : state.stepsOut * 1.25;
